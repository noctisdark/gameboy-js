...
ROMCartridge.from = function (cartridge) {
  ...
  if (supported.indexOf(type) == -1) throw "Unimplemented";
...
export class MBC1Cartridge extends ROMCartridge {
  constructor() {
    super();
    this.name = "MBC1";
    this.ram = [];
...
class MBC2Cartridge extends ROMCartridge {
  ...
  //bad branch -- try to find another solution
  ramGet(x) {
    if (!this.ramEnabled || x > 0x1ff) return 0xff;

    return this.ram[this.RAMOff + (x & 0x1fff)];
  }

  ramSet(x, y) {
    if (!this.ramEnabled || x > 0x1ff) return;
