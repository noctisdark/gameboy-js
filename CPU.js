class CPU {
    constructor(system) {
        //main memory
        this.system = system; system.cpu = this;
    ...
    get H() { return (this.RF[1] & 0x20) >> 5; }
    get C() { return (this.RF[1] & 0x10) >> 4; }
    ...
    set flag(x) { this.RF[1] = (x << 4) }
    get flag() { return this.RF[1] >> 4; }

    get IF() { return this.memory.get(0xff0f); }
    get IE() { return this.memory.get(0xffff); }

    set IF(x) { this.memory.set(0xff0f, x); }
    set IE(x) { this.memory.set(0xffff, x); }


    get HL() { return this.RF[7] | (this.RF[6] << 8); }
    get BC() { return this.RF[3] | (this.RF[2] << 8); }
    get DE() { return this.RF[5] | (this.RF[4] << 8); }
    get AF() { return this.RF[1] | (this.RF[0] << 8); }
    ...
    get16() {
        let l = this.memory.get(this.PC++), h = this.memory.get(this.PC++);
        return l | (h << 8);
    }
    ...
    daa() {
        let x = this.RF[0],
            a = (x & 0xf0) >> 4, b = x & 0xf,
            correction = 0, n = this.N, h = this.H, c = this.C, z;
    ...
    interruptProcedure(addr) {
        ...
        this.push16(this.PC); this.cycles += 8;
    ...
    step() {
        ...
        if ( this.remainingCycles < 0 )
            this.cycles = - this.remainingCycles;
