class Joypad {
	...
	updateState(idx, val) {
		...
		if ( idx < 4 && !button && bit && !val )
			this.system.requestInterrupt(4);
		if ( idx >= 4 && !direction && bit && !val )
			this.system.requestInterrupt(4);
	...
	applyState() {
		...
		if (!button) //button selections
			this._register &= (this.state & 0xf);

		if (!direction) //button selections
		...
		switch (e.keyCode) {
			case 90:
				for ( let handle of this.handles )
					handle.updateState(6, !press);
				break;

			case 68:
				for ( let handle of this.handles )
					handle.updateState(4, !press);
				break;

			case 83:
				for ( let handle of this.handles )
					handle.updateState(7, !press);
				break;

			case 81:
				for ( let handle of this.handles )
					handle.updateState(5, !press);
				break;

			case 65:
				for ( let handle of this.handles )
					handle.updateState(0, !press);
				break

			case 69:
				for ( let handle of this.handles )
					handle.updateState(1, !press);
				break

			case 79:
				for ( let handle of this.handles )
					handle.updateState(2, !press);
				break

			case 80:
				for ( let handle of this.handles )
					handle.updateState(3, !press);
				break
